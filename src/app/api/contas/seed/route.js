...
export async function POST(request) {
  try {
    ...
    const existentes = await prisma.conta.count({ where: { empresaId, ativa: true } });
    if (existentes > 0) {
      return NextResponse.json({ erro: 'Empresa já possui plano de contas' }, { status: 409 });
    }
    ...
    return NextResponse.json({ mensagem: `Plano de contas CFC carregado com sucesso! ${count} contas inseridas.`, total: count }, { status: 201 });
  } catch (error) {
    console.error('Erro ao carregar seed:', error);
    return NextResponse.json({ erro: 'Erro ao carregar plano de contas CFC' }, { status: 500 });
  }
}
